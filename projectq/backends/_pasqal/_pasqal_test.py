...
"""Tests for projectq.backends._pasqal._pasqal.py."""

import pytest
import math

from projectq import MainEngine
from projectq.backends._pasqal import _pasqal
...
def test_pasqal_backend_is_available(single_qubit_gate, is_available):
    eng = MainEngine(backend=DummyEngine(), engine_list=[DummyEngine()])
    qubit1 = eng.allocate_qubit()
    pasqal_backend = _pasqal.pasqalBackend()
    cmd = Command(eng, single_qubit_gate, (qubit1, ))
    assert pasqal_backend.is_available(cmd) == is_available
...
@pytest.mark.parametrize("num_ctrl_qubits, is_available", [(0, True),
                                                           ...
                                                           (2, False),
                                                           (3, False)])
def test_pasqal_backend_is_available_control_not(num_ctrl_qubits, is_available):
    eng = MainEngine(backend=DummyEngine(), engine_list=[DummyEngine()])
    qubit1 = eng.allocate_qubit()
...
def test_pasqal_backend_init():
    backend = _pasqal.pasqalBackend(verbose=True, use_hardware=True)
    assert len(backend._circuit) == 0


def test_pasqal_empty_circuit():
    backend = _pasqal.pasqalBackend(verbose=True)
    eng = MainEngine(backend=backend)
    eng.flush()


def test_pasqal_invalid_command():
    backend = _pasqal.pasqalBackend(verbose=True)
...
def test_pasqal_sent_error(monkeypatch):
    # patch send
    def mock_send(*args, **kwargs):
        raise TypeError

    monkeypatch.setattr(_pasqal, "send", mock_send)
...
def test_pasqal_too_many_runs():
    backend = _pasqal.pasqalBackend(num_runs=300, verbose=True)
...
def test_pasqal_retrieve(monkeypatch):
    # patch send
    def mock_retrieve(*args, **kwargs):
        return [0, 6, 0, 6, 0, 0, 0, 6, 0, 6]

    monkeypatch.setattr(_pasqal, "retrieve", mock_retrieve)
    backend = _pasqal.pasqalBackend(
...
def test_pasqal_backend_functional_test(monkeypatch):
    correct_info = {
        'circuit':
    ...
    backend = _pasqal.pasqalBackend(verbose=True, num_runs=10)
